<?php
	...
	if(isset($_REQUEST['ttid'])){
		$tr_type = mysqli_escape_string($conn, $_REQUEST['ttid']);
	...
	<table class="table table-striped table-hover p-1 paragraph-text">
		...
		?>
			<tr>
			  ...
			  <td><a href="edit-record.php?tr_id=<?php echo $record_row['tr_id'];?>"><?php echo aes_decrypt($record_row['tr_title']);?></a></td>
			  <td><?php echo aes_decrypt($record_row['tr_desc']);?></td>
			  <td><?php echo aes_decrypt($record_row['tr_doctor']);?></td>
			  <td><?php echo aes_decrypt($record_row['tr_files']);?></td>
			  <td><?php echo "Rs. ".aes_decrypt($record_row['tr_cost'])." /-";?></td>
			</tr>
		<?php
			$sr = $sr++;
				}
